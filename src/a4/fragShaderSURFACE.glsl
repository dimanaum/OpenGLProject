...
void main(void)
{
	vec4 fogColor = vec4(0.2, 0.2, 0.7, 1.0);
	float fogStart = 0.2;
	float fogEnd = 80;

	vec4 fogColor2 = vec4(0.2, 0.2, 0.7, 1.0);
	float fogStart2 = 0.2;
	float fogEnd2 = 80;
	...
	float dist2 = length(vertEyeSpacePos.xyz);
	float fogFactor2 = clamp(((fogEnd2-dist2)/(fogEnd2-fogStart2)), 0.0, 1.0);

	// normalize the light, normal, and view vectors:
	vec3 L = normalize(light.position - vertEyeSpacePos);
	vec3 V = normalize(-vertEyeSpacePos);
	...
	if(fogMode != 0) color = mix(fogColor2, color, fogFactor2);
	else if (isAbove != 1) color = mix(fogColor, color, pow(fogFactor,5));
